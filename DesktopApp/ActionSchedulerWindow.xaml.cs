...
namespace ListViewDragDropManagerDemo
{
    ...
    {
        ListViewDragDropManager<Worker> dragMgr;
        ListViewDragDropManager<Worker> dragMgr2;
        ...
        void ActionSchedulerWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            ObservableCollection<Worker> tasks = Worker.CreateTasks();
            this.listView.ItemsSource = tasks;

            this.listView2.ItemsSource = new ObservableCollection<Worker>();

            // This is all that you need to do, in order to use the ListViewDragManager.
            this.dragMgr = new ListViewDragDropManager<Worker>(this.listView);
            this.dragMgr2 = new ListViewDragDropManager<Worker>(this.listView2);
        ...
        void dragMgr_ProcessDrop(object sender, ProcessDropEventArgs<Worker> e)
        {
        ...
        void OnListViewDrop(object sender, DragEventArgs e)
        {
            ...
            Worker task = e.Data.GetData(typeof(Worker)) as Worker;
            if (sender == this.listView)
            {
                ...
                (this.listView2.ItemsSource as ObservableCollection<Worker>).Remove(task);
            }
            else
            {
                ...
                (this.listView.ItemsSource as ObservableCollection<Worker>).Remove(task);
            }
        }
